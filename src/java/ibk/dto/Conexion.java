...
public class Conexion {
    ...
    public String getFilesData(int cod) {
        String path = "";
        boolean ok = false;
        boolean ok1 = false;
        byte[] fileBytes;
        ...
        Connection conn = getConnection();
        try {
            query = "select * from BD_CHIP.Phoenix.Imagenes where cod=" + cod;
            query1 = "select * from BD_CHIP.Phoenix.Formulario where Id=" + cod;
            Statement state = conn.createStatement();
            Statement state1 = conn.createStatement();
            ResultSet rs = state.executeQuery(query);
            ResultSet rs1 = state1.executeQuery(query1);
            File dir = new File("\\\\hipotecario\\Solicitudes\\solicitud" + cod);
            dir.mkdir();
            int read = 0;
            while (rs.next()) {
    ...
    public int contarFiles(int cod) {
        int cont = 0;
