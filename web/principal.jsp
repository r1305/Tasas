...
<html>
    ...
    <body>
        ...
        <sql:query dataSource="${snapshot}" var="repe_red">
            ...
            left join [BD_CHIP].[Phoenix].[Usuarios] u 
            on (f.Usuario=u.Registro) 
            where f.Estado='Pendiente' and u.Canal2='Red' and f.Cont=2
            order by tiempo desc
        </sql:query>
        ...
        <sql:query dataSource="${snapshot}" var="repe_ffvv">
            SELECT *,
            case Moneda
            when 'Dolares' then '$ '+replace(convert(nvarchar(20),convert(money,round(Prestamo,0,0)),1),'.00','')
            when 'Soles' then 'S/. '+replace(convert(nvarchar(20),convert(money,round(Prestamo,0,0)),1),'.00','') 
            ...
            left join [BD_CHIP].[Phoenix].[Usuarios] u 
            on (f.Usuario=u.Registro) 
            where f.Estado='Pendiente' and u.Canal2='FFVV' and f.Cont=2
            order by tiempo desc
        </sql:query>
        ...
        <!-- Query para contar las solicitudes rechazadas según encargado -->
    ...
    <center>
        <div class="container" align="center" style="width: 88.5%">
            ...
            <ul class="nav nav-tabs nav-justified">
                ...
                <li><a data-toggle="tab" href="#repe_red" style="color: #0060B3;font-size: 13px"><b>Repechaje - Red (<c:forEach var="b" items="${repe_red_n.rows}">${b.numero}</c:forEach>)</b></a></li>
                <li><a data-toggle="tab" href="#repe_ffvv" style="color: #0060B3;font-size: 13px"><b>Repechaje - FFVV (<c:forEach var="b" items="${repe_ffvv_m.rows}">${b.numero}</c:forEach>)</b></a></li>
                ...
                <!-- Solicitudes repechajes de RED -->
                <div id="repe_red" class="tab-pane fade">
                                    ...
                                    <tr style="text-align: center">
                                        <td style="font-size: 12px;vertical-align:middle;" width="10%"><a href="inicio_ffvv.jsp?id=${a.Id}">${a.solicitud}</a>
                                            <c:choose>
                                                ...
                                                <c:when test="${a.tiempo>480  && a.tiempo<=1260}">
                                                    <span align="center" style="color:#FACC2E; font-family: Webdings; font-weight:bold">n</span>
                                                </c:when>
                                                <c:when test="${a.tiempo>1260}">
                                                    <span align="center" style="color:red; font-family: Webdings; font-weight:bold">n</span>
